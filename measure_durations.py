...
for test in tests[:10]:
    print(test)
    try:
        output = benchmark([sys.executable, "-m", "test", test])
        memray_output = benchmark([sys.executable, "-m", "memray", "run", "-fo", "/dev/null", "-m", "test", test])
    except subprocess.SubprocessError:
        pass
    results.append((test, output, memray_output))
