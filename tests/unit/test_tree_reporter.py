...
class TestTreeTui:
    def test_no_allocations(self):
        ...
        assert tree_to_dict(root) == TreeElement(
            label="<No allocations>", children=[], is_expanded=True
        )
    ...
    def test_select_node_and_get_more_information(self):
        # GIVEN
        peak_allocations = [
            MockAllocationRecord(
                tid=1,
        ...
        async def run_test():
            async with app.run_test() as pilot:
                ...
                child = tree.root.children[0].children[0].children[0]
                tree.select_node(child)
                await pilot.press("s")
                await pilot.pause()
                return app.screen

        select_screen = async_run(run_test())

        # THEN
        assert select_screen.frame == Frame(
            location=("me2", "fun2.py", 12),
            value=10240,
            children={},
            n_allocations=1,
            thread_id="0x1",
    ...
    def test_select_node_and_get_more_information_any_key_exits(self):
        # GIVEN
        peak_allocations = [
            MockAllocationRecord(
                tid=1,
        ...
        async def run_test():
            async with app.run_test() as pilot:
                ...
                await pilot.press("p")
                await pilot.pause()
                return first_screen, second_screen, app.screen

        first_screen, second_screen, third_screen = async_run(run_test())

        # THEN
        assert first_screen != second_screen
        assert first_screen == third_screen

    def test_select_node_and_get_more_information_no_node_selected(self):
        # GIVEN
        peak_allocations = [
            MockAllocationRecord(
                tid=1,
        ...
        # THEN
        # Check that we are getting information about the root
        assert select_screen.frame.location == ("<ROOT>", "", 0)
    ...
    def test_hide_import_system(self):
        ...
        first_root, second_root = async_run(run_test())

        # THEN
        assert tree_to_dict(first_root) == TreeElement(
            label="📂 11.000KB (100.00 %) <ROOT>  ",
            children=[
                TreeElement(
                    label="📂 10.000KB (90.91 %) grandparent2  fun2.py:4",
                ...
                TreeElement(
                    label="📂 1.000KB (9.09 %) grandparent  <frozen importlib>:4",
                    children=[
                        TreeElement(
                            label="📂 1.000KB (9.09 %) parent  <frozen importlib>:8",
                            children=[
                                TreeElement(
                                    label="📄 1.000KB (9.09 %) me  <frozen importlib>:12",
                                    children=[],
                                    is_expanded=False,
                                )
                            ],
        ...
        assert tree_to_dict(second_root) == TreeElement(
            label="📂 11.000KB (100.00 %) <ROOT>  ",
            children=[
                TreeElement(
                    label="📂 10.000KB (90.91 %) grandparent2  fun2.py:4",
    ...
    def test_hide_import_system_idempotency(self):
        ...
        # THEN
        assert tree_to_dict(first_root) == tree_to_dict(second_root)
...
class TestTUILooks:
    ...
    def test_select_screen(self, tmp_path, compare):
        ...
        with patch("linecache.getlines") as getlines:
            ...
            assert compare(
                peak_allocations, terminal_size=(80, 60), press=[*["down"] * 3, "s"]
            )
